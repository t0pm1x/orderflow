...
| v1.2.reaudit | senior-go adversarial re-audit — F-001 (web SPA embed symbols unexported; build broken) + F-002 (saga cmd test race vs httpSrv.Serve Accept) + F-003 (Makefile GOTOOLCHAIN for hosts <1.25.13) + F-004 (web SPA JS bundles 404; embed prefix misunderstanding) + F-005 (order chips routed to `/` not `/orders`) + F-006 (payment-sim select bind machinery + chunk-cascade) + F-007 (BFF `/api/orders` empty response = `{"items":null}` → SPA `[...null]`) + F-008 (payment webhook 404 when saga hasn't pre-created row → "Force succeed/fail" broken pre-saga; fixed via UpsertFromWebhook mock-provider semantics) + F-009 (F-008 INSERT failed with empty order_id against UUID-typed column → 502; fixed via PGRepo default + SPA explicit field); build/test/race/vet/e2e-happy all green; 16/16 SPA assets serve correctly | done | 929e6dd, c36ff53, f4f3083, c739d69, d0a3a55, 3d75eca, de3039e, 7476ca9, d21da3d, (F-009) | docs/superpowers/plans/2026-08-27-orderflow-reaudit.md |
