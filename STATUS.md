...
| v1.2.reaudit | senior-go adversarial re-audit — F-001 (web SPA embed symbols unexported; build broken) + F-002 (saga cmd test race vs httpSrv.Serve Accept) + F-003 (Makefile GOTOOLCHAIN for hosts <1.25.13) + F-004 (web SPA JS bundles 404; embed prefix misunderstanding) + F-005 (order chips routed to `/` not `/orders`) + F-006 (payment-sim select bind machinery + chunk-cascade) + F-007 (BFF `/api/orders` empty response = `{"items":null}` → SPA `[...null]`); build/test/race/vet/e2e-happy/e2e-compensation all green; 16/16 SPA assets serve correctly; new regression test `TestAPI_ListOrders_NilItemsCoercedToEmptyArray` added | done | 929e6dd, c36ff53, f4f3083, c739d69, d0a3a55, 3d75eca, de3039e, (F-007) | docs/superpowers/plans/2026-08-27-orderflow-reaudit.md |
