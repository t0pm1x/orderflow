/*
Server Sent Events Extension (htmx-ext-sse v2.2.4)
=================================================
Vendored from https://cdn.jsdelivr.net/npm/htmx-ext-sse@2.2.4/dist/sse.min.js
to replace the htmx-1.x SSE plugin that was generating a runtime
console warning ("You are using an htmx 1 extension with htmx 2.0.3")
and was incompatible with the current htmx core. Same usage shape:
the element carrying `hx-ext="sse" sse-connect="..."` is the source;
child elements with `sse-swap="<EventName>"` are swapped into on
message receipt. See services/web/internal/templates/layout.html
for the only call site.

Local modifications vs upstream:
* none — byte-for-byte copy of the published 2.2.4 minified bundle
  so a future `go mod tidy`/dependency bump against npm doesn't have
  to re-vendor.
*/
